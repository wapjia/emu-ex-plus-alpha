...
void SaturnSystem::onSessionOptionsLoaded(EmuApp &app)
{
	...
	//爱吾修改：增加默认BIOS路径
	if(naBiosPath.empty()){
		naBiosPath = std::string(FS::pathString(appContext().supportPath(), "mpr-17933.bin"));
	}
	if(jpBiosPath.empty()){
